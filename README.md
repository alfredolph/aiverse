...
下载 `AIVerse-Setup-1.0.5.exe` 运行安装向导。默认装到
...
# -> installer/Output/AIVerse-Setup-1.0.5.exe
```
...
- **修（自己引入又自己抓住的）**：给 `detect()` 加缓存时把「不可用」也缓存了。
  于是 `planner.build_plan()` 在部署**开始前**那次探测（那时确实还没有），
  会把「FFmpeg 未安装」钉住 30 秒 —— 而一键部署 4 秒就装完了，
  界面于是继续显示「未安装」，导出也一直失败。现在**只缓存「可用」的结果**；
  「不可用」时根本没起子进程（只是几个 `Path.exists()`），不缓存也没有代价。
  这条是 CI 里 `tools/test_export.py` 红的。
...
起因是把管线跑满之后，认真推演了一遍「改一处会发生什么」，发现有两件事对不上。

- **修（核心语义）**：「返回修改」只改当前阶段，**下游全都不动**。
  把「角色设计」退回修改后，场景 / 分镜 / 视频 / 配音 / 剪辑 / 成片在界面上
  依旧是一片「已通过」—— 可它们全是拿旧角色资产生成的。用户以为只改了一处，
  实际后面每一段都建立在已经失效的输入上。
  现在 `reopen()` 默认**级联**把所有下游阶段退回草稿（产物文件保留，只是不再算
  「已通过」），并在返回体里列出被作废的阶段；前端会先把「哪些阶段要一起退回」
  讲清楚再动手。传 `{"cascade": false}` 可以只改当前阶段状态。
- **修（审核门被绕过）**：`gate()` 原来**只看紧邻的上一阶段**。于是「角色回退之后，
  分镜照样能直接跑」—— 因为分镜的前一段（场景）还是 APPROVED，门就开了。
  现在逐个检查**所有**上游阶段，任何一段没通过都挡住；侧栏那句「需先通过上一阶段」
  也改成指向真正卡住的那一段。
- **修**：任务接口「假成功」。`/api/tasks/{id}/cancel` 和 `/bump` 对**不存在**的任务
  也回 `{"ok": true}`，前端于是弹「已取消」。现在如实报「任务不存在」；对已经结束的
  任务也不再假称取消成功。
- **修**：取消正在运行的任务其实无效。原来只把数据库状态改成 `cancelled`，worker 跑完
  还会覆写成 `done`，用户看到的「已取消」下一秒就变回去。现在改成**协作式取消**：
  任务的 `progress()` 每报一次进度就检查一次，已取消则抛 `TaskCancelled`，
  由 worker 统一收尾（不会留下写了一半的文件）。
- **修**：非法请求体会被当成空参数。`_read_body()` 解析失败时静默返回 `{}`，
  于是 `POST /api/projects` 收到一段坏 JSON 会**凭空建出一个「未命名项目」**，
  客户端那边只看到请求成功。现在解析失败一律 400，错误停在边界上。
- **修**：注定失败的任务会被排进队列。`video/generate` 的 `only` 里写个不存在的镜号，
  任务照排，跑到一半抛「镜头 S005 不存在」—— 用户得去队列抽屉里翻才知道哪写错了，
  而且整批中断、前面已生成的镜头白跑。现在入队前先校验镜号（越界 / 非数字都当场
  400 + 可读提示），并且单个镜头失败不再拖垮整批，失败的记进 `failed` 继续跑。
- **修**：资产快照接口 500。`entities.snapshot()` 把**同一个** payload 塞进快照，
  随后又把 `_versions` 挂回这个 payload —— 自引用，`json.dumps` 抛
  `Circular reference detected`。这个洞一直没露头，是因为它只在「实体真的存在」时
  才会走到，而当时测试里的工作项目恰好一个实体都没有、接口提前 return 了。
  修好上面那条测试夹具之后，它当场就现形了。
- **修**：Provider 类型写错会被照单收下。`POST /api/providers` 传个不存在的
  `type`，记录进了库、界面回「添加成功」，但那条记录**在界面上根本不存在**
  （`list_providers` 只遍历已知类型，`_user_providers` 里的 `KeyError` 又被
  吞掉）。现在当场 400 并列出可选类型，`PATCH` 改类型同理。
- **修**：前端 `gacha-adopt` 的成功提示是个空壳
  （`已采用方案 ${r.design?.name ? '' : ''}` —— 三元两边都是空串），弹出来永远是
  「已采用方案 」。改成用后端返回的实体名。
- **修（测试自己在空转）**：`tools/test_api_surface.py` 建的「工作项目」**没有给剧本**，
  于是「跑满阶段」那一轮每个阶段都静默失败，这个项目其实一个镜头都没有 ——
  队列暂停那条断言看着过了，其实是在空跑。现在补上剧本，并把「阶段到底跑起来没有」
  「分镜有没有真的建出来」显式断言出来。
- **加**：行为断言从 5 组扩到 9 组，新增的都是这次修的东西的回归：级联作废、
  审核门挡住失效上游、任务接口不假成功、非法输入停在边界（坏 JSON 不建项目、
  越界镜号不排任务）。

> 这一版没有界面功能变化（「返回修改」多了一个确认框，侧栏提示更准确），
> 但**建议升级**：v1.0.4 及更早版本里，「返回修改」只改一个阶段的状态、下游继续标着
> 「已通过」，而且审核门只看紧邻一段 —— 很容易在不知情的情况下拿旧资产继续往下跑，
> 等出片了才发现对不上。
